...
public final class Minefield {
	...
	private static final EnumSet<State> playStates = of(State.START,
			State.PLAYING);
	private static final Logger logger = Logger
			.getLogger("org.foobar.minesweeper.model.minefield");
	 ...
	 * 
	 * <p>
	 * Calling this method repeatedly with the same row and column will no have
	 * no effect until restart() is called.
	 * </p>
	 * 
	 * @param row
	 *            the row whose cell will be revealed.
	 * @param column
	 *            the column whose cell will be revealed.
	 * 
	 ...
	 * 
	 * @param row
	 *            the row whose flag will be toggled.
	 * @param column
	 *            the column whose flag will be toggled.
	 * 
	 * @throws IndexOutOfBoundsException
	 *             if (@code row} is negative or greater than or equal to
	 *             {@code getRowCount()}
	...
	public void revealNearby(int row, int column) {
		...
		checkState(playStates.contains(gameState)
				&& entry.cell.getType().hasMineCount());
